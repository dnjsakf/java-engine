...
public class DQLService extends DBManager {
    ...
    // 데이터 조회 함수
    public Map<String, Object> getLastArticle(Map<String, Object> dataMap){
        // 상수설정
        //   - SQL
        final String SQL = "SELECT IFNULL(MAX(T1.ATCL_ID), 0) AS MAX_ATCL_ID  "+"\n"
                         + "     , IFNULL(MIN(T1.PAGE), -1)    AS MAX_PAGE     "+"\n"
                         + "  FROM CW_BLOG_ATCL_LIST T1                       "+"\n"
                         + " WHERE 1=1                                        "+"\n"
                         + "   AND T1.BLOG_ID = ?                             "+"\n"
                         + "   AND T1.CATE_ID = ?                             "+"\n"
        ...
        try {
            ...
            if( rs.next() ) {
                ...
                for(String column : columnNames) {
                    selected.put(column, rs.getObject(column));
                }
            }
